...
# PPF1.0 consists of 6-bytes PPF version number, 50-byte description and the remaining bytes is the patch data
...
# PPF2.0 consists of 6-bytes PPF version number, 50-byte description, 4-bytes bin file size, 1024-byte block-check and the remaining bytes is the patch data
...
def apply_ppf3_patch(ppf_file, bin_file, mode: int = 1):
    ...
    if blockcheck:
        ...
        # Compare the 1024-byte block data from the PPF patch file with the block of data from the BIN/ISO file
        if ppfblock != binblock:
            print("Warning: Binblock/Patchvalidation failed, continuing anyway")
